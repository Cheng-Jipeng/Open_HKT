#= ================================================================
   TwoCountryProductionOLG.jl  –  Zero-exponent types & helpers for the
   ...
   - ν_b = ξ_W = 0 are fixed primitives;
   - the absorbing system has seven unknowns and seven equilibrium equations:
       no common-growth residual, exponent unknown, or exponent root is present;
   - one normalized absorbing equilibrium is solved per parameterization and
       restated at each switch state with q_i,d_i proportional to 1/N_i
   ================================================================ =#
...
# 1. Parameters
#
# Except for the requested zero absorbing exponents, the DEFAULTS reproduce the
...
# while this workflow sets the fixed primitives ν_b=ξ_W=0.
...
#
# At ν_b=ξ_W=0, the separate common-world-growth condition is the identity
# G_N_US^0 = G_N_W^0 = 1. It is therefore neither an economic restriction nor
# an equation in the nonlinear system. `common_world_growth` is retained only
# as a legacy compatibility marker; it never enters an equilibrium residual.
# ─────────────────────────────────────────────────────────────────
...
@with_kw struct ProductionParams
    ...
    ν_b::Float64       = 0.00          # zero-exponent specialization; never an unknown
    ...
    ξ_W::Float64      = 0.00           # zero-exponent specialization; never an unknown
    ...
    # Legacy inert metadata. With zero exponents, common growth is already the
    # identity 1 == 1, so this value never enters validation or any residual.
...
function validate_params(p::ProductionParams)
    ...
    @assert p.ν_b == 0.0                              "zero-exponent solver requires ν_b = 0"
    @assert p.ξ_W == 0.0                              "zero-exponent solver requires ξ_W = 0"
    ...
    _require_zero_nu_b(p)
    nothing
end

# Kept separate from `validate_params` because notebook helpers sometimes call
# `solve_bgp_at` directly. The legacy common-growth flag is deliberately not
# inspected: at zero exponents it cannot add a restriction or residual.
function _require_zero_nu_b(p::ProductionParams)
    (p.ν_b == 0.0 && p.ξ_W == 0.0) || throw(ArgumentError(
        "This solver is specialized to ν_b = ξ_W = 0.",
    ))
    return nothing
end
...
function productivity_US(p::ProductionParams, regime::Symbol, N_US)
    if regime === :b
        return (p.Abar_X_US, p.Abar_L_US)
    elseif regime === :u
...
struct BGPResult
    ...
    ν_b_eff::Float64                         # compatibility constant; always 0, never solved
...
"""
    _restate_zero_bgp(p, template, N_US, N_W)

Materialize the single zero-exponent absorbing equilibrium at a new inherited
knowledge state. Policies, returns, wages, income, aggregate market values, and
aggregate dividends are copied exactly. Only the stored state and per-variety
objects change:

    q_i(N_i) = q_i(template) * template.N_i / N_i,
    d_i(N_i) = d_i(template) * template.N_i / N_i.

This is a deterministic rescaling, not an equilibrium solve. The compatibility
field `ν_b_eff` is fixed at zero and is not an unknown or a switch-specific
quantity.
"""
function _restate_zero_bgp(p::ProductionParams, template::BGPResult, N_US, N_W;
                           validate::Bool=true)
    _require_zero_nu_b(p)
    all(isfinite, (N_US, N_W, template.N_US, template.N_W)) ||
        throw(ArgumentError("absorbing knowledge states must be finite"))
    (N_US > 0 && N_W > 0 && template.N_US > 0 && template.N_W > 0) ||
        throw(ArgumentError("absorbing knowledge states must be positive"))

    q_US = template.q_US * template.N_US / N_US
    q_W  = template.q_W  * template.N_W  / N_W
    d_US = template.d_US * template.N_US / N_US
    d_W  = template.d_W  * template.N_W  / N_W

    x_c = _bgp_pack(
        template.φ_US, template.φ_W, template.ω, template.θ_US_star,
        template.ω_star, template.R_f, template.R_f_W, p.φ_floor,
    )
    F = zeros(7)
    bgp_residual!(F, x_c, p, N_US, N_W)
    residual_norm = all(isfinite, F) ? norm(F) : Inf
    is_converged = template.converged && residual_norm < 1e-5

    result = BGPResult(
        template.φ_US, template.φ_W,
        template.ω, template.ω_star,
        template.θ, template.θ_US_star,
        template.R_f, template.R_f_W,
        Float64(N_US), Float64(N_W),
        template.Y_US, template.Y_W,
        template.e_US, template.e_W,
        q_US, q_W, d_US, d_W,
        template.Q_US, template.Q_W,
        template.I_US, template.I_W,
        template.R_US, template.R_W,
        template.R_p, template.R_A,
        template.R_p_star, template.R_A_star,
        template.G_N_US, template.G_N_W, 0.0,
        template.Psi,
        is_converged, residual_norm,
    )

    if validate && is_converged
        _check_v43_identities(
            "BGP(US)", result.Y_US, result.e_US, result.N_US,
            result.d_US, result.I_US, result.G_N_US, result.q_US,
        )
        _check_v43_identities(
            "BGP(W)", result.Y_W, result.e_W, result.N_W,
            result.d_W, result.I_W, result.G_N_W, result.q_W,
        )
    end
    return result
end
...
"""
Solve the seven-equation absorbing system once at the normalized state
`(N_US,N_W)=(1,1)`, then restate it at the requested inherited state.

There is no common-growth residual, exponent unknown, or exponent root.
"""
function solve_bgp_at(p::ProductionParams, N_US, N_W; x0_actual=nothing, verbose=false)
    _require_zero_nu_b(p)
    (isfinite(N_US) && isfinite(N_W) && N_US > 0 && N_W > 0) ||
        throw(ArgumentError("absorbing knowledge states must be finite and positive"))
    normalized_N_US = 1.0
    normalized_N_W = 1.0
    ...
    for x0 in candidate_starts
        ...
        for method in (:trust_region, :newton)
            try
                r = nlsolve((F, x) -> bgp_residual!(
                                F, x, p, normalized_N_US, normalized_N_W),
    ...
    if best_x === nothing
        ...
        F0 = zeros(7)
        bgp_residual!(F0, x0_c, p, normalized_N_US, normalized_N_W)
        verbose && @warn "Normalized absorbing solve failed; returning initial guess"
        normalized = _bgp_make_result(
            p, normalized_N_US, normalized_N_W, x0_c, false, norm(F0);
            validate=false,
        )
        return _restate_zero_bgp(p, normalized, N_US, N_W; validate=false)
    end

    normalized = _bgp_make_result(
        p, normalized_N_US, normalized_N_W, best_x, best_conv, best_norm,
    )
    return _restate_zero_bgp(p, normalized, N_US, N_W)
end


# ─────────────────────────────────────────────────────────────────
# 7. Zero-exponent absorbing-state selection
#    One normalized seven-equation BGP is solved per parameterization. There is
#    no common-growth residual or exponent root; switch states are restated by
#    the exact per-variety 1/N scaling.
# ─────────────────────────────────────────────────────────────────

"""
Compatibility stub for notebooks written against the common-growth solver.
The zero-exponent implementation has no exponent-calibration equation and
therefore rejects calls to `calibrate_common_growth` explicitly.
"""
function calibrate_common_growth(args...; kwargs...)
    throw(ArgumentError(
        "calibrate_common_growth is unavailable: ν_b = ξ_W = 0 makes " *
        "common growth the identity 1 = 1, not an equilibrium equation.",
    ))
end

"""
Solve the normalized seven-equation absorbing BGP and algebraically restate it
at a predetermined state. `BGPResult.ν_b_eff` is retained only as a constant
zero compatibility field; it is never an unknown.
"""
...
# Legacy compatibility hook; the zero-exponent branch solver does not call it.
...
"""
Residual of the reduced seven-equation BGP system when the stored allocations
of `result` are re-evaluated at `(N_US,N_W)`. This verifies the algebraically
restated record without adding a root solve.
"""
...
"""
Materialize the one normalized absorbing equilibrium over a vector of inherited
states. This performs only algebraic `1/N` rescaling and residual verification;
it never invokes `nlsolve`.
"""
function _materialize_zero_bgp_sequence(p::ProductionParams,
                                        template::BGPResult,
                                        N_US_path::AbstractVector,
                                        N_W_path::AbstractVector)
    length(N_US_path) == length(N_W_path) ||
        throw(DimensionMismatch("US and RoW knowledge paths must have equal length"))
    sequence = [
        _restate_zero_bgp(p, template, N_US_path[t], N_W_path[t])
        for t in eachindex(N_US_path)
    ]
    for t in eachindex(sequence)
        _usable_bgp_continuation(
            p, sequence[t], N_US_path[t], N_W_path[t],
        ) || error("Algebraically restated absorbing equilibrium failed at t=$(t).")
    end
    return sequence
end
...
#    The zero-exponent switch successor is supplied through its algebraically
#    restated per-variety prices (for use within forward-backward iteration).
# ─────────────────────────────────────────────────────────────────
...
# Construct just the switch-successor fields used by the u residual. This
# helper is generic in the knowledge-state number type, so ForwardDiff can
# propagate derivatives through q_i,d_i ∝ 1/N_i during the global polish.
function _zero_bgp_successor(template::BGPResult, N_US, N_W)
    return (
        q_US=template.q_US * template.N_US / N_US,
        d_US=template.d_US * template.N_US / N_US,
        q_W=template.q_W * template.N_W / N_W,
...
Inputs:
  ...
  next_b                 : algebraically restated b-successor prices at
                           (N_US_t+1^u, N_W_t+1)
"""
function u_residual!(F, x_c, p::ProductionParams,
    ...
    # The switch prices come from the single normalized absorbing equilibrium,
    # restated at this implied successor state; no root is solved here.
...
#            restate the normalized absorbing solution at successor states,
...
function _u_branch_global_residual!(F, x, p::ProductionParams,
    ...
    for t in 1:T
        ...
        next_b = _zero_bgp_successor(bgp_template, N_US[t+1], N_W[t+1])
        x_t = x[7*(t-1)+1:7*t]
...
function _polish_u_branch_global(p::ProductionParams,
    ...
    try
        r = nlsolve((F, x) -> _u_branch_global_residual!(
                        F, x, p, bgp_template, pol_terminal),
...
- Restate the one normalized absorbing equilibrium at each inherited state;
  no absorbing root is solved inside the iteration.
...
function solve_unbalanced_branch(p::ProductionParams,
    ...
    # Materialize state records from the single normalized absorbing solution.
    # A warm absorbing sequence is intentionally unnecessary: no date-specific
    # root is solved in the zero-exponent model.
    initial_bgp_seq !== nothing && verbose && println(
        "  ignored absorbing-sequence warm start (zero-exponent solve-once mode)",
    )
    bgp_seq = _materialize_zero_bgp_sequence(
        p, bgp_init, N_US_path, N_W_path,
    )
    ...
    for outer_it in 1:p.branch_iters
        ...
        # ---- Step 2: algebraically restate the normalized absorbing solution ----
        bgp_seq = _materialize_zero_bgp_sequence(
            p, bgp_init, N_US_path, N_W_path,
        )
        n_bgp_failed = 0
    ...
    # Expensive (Jacobian is 7T × 7T). The normalized absorbing policy template
    # is fixed, while switch q,d are differentiated through the current knowledge
    # states. The final residual rebuild is the authoritative convergence check.
    if p.do_global_polish && (!converged_flag || any(>(1e-5), filter(isfinite, last_residual_norms)))
        ...
        polished = _polish_u_branch_global(p, pol, bgp_init, verbose=verbose)
        if polished !== nothing
            pol = polished
            converged_flag = true
        end
...
function build_switch_path(result::ProductionSimulationResult, τ::Int; T::Int=length(result.u_path))
    ...
    if τ ≤ T
        ...
        template = first(result.bgp_seq)

        for t in τ:T
            b = _restate_zero_bgp(p, template, N_US, N_W)
...
2. Use ν_b = ξ_W = 0, so common growth is the identity 1 = 1 rather than an
   equilibrium equation.
3. Solve one normalized absorbing equilibrium via the reduced 7-equation
   Markov competitive equilibrium system
   (V4_3 lines 818–855).
4. Solve the unbalanced branch by forward-backward iteration, algebraically
   restating the normalized absorbing equilibrium at switch states.
...
function run_production_simulation(p::ProductionParams=ProductionParams();
    ...
    p_use = p
    verbose && println("Solving one normalized absorbing equilibrium with ν_b = ξ_W = 0...")
...
#     ProductionParams DEFAULTS are the zero-exponent experiment (see §1).
...
function fast_growth_params(; T_max::Int=20, n_buffer::Int=-1,
    ...
    ProductionParams(;
        ...
        Abar_X_US=1.0, Abar_L_US=1.0, ν_b=0.0,
        N_US_0=1.0,
        # ── RoW (original) ──
        α_W=0.30, ρ_W=1.50, ϑ_W=0.66, a_W=1.00, H_W=1.0, L_W=1.0,
        Abar_X_W=1.0, Abar_L_W=1.0, ξ_W=0.0, N_W_0=1.0,
