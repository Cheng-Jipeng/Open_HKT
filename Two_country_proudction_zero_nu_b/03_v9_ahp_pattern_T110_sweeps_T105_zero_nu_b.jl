# This file is included after 03_v9_ahp_pattern_T110_replication_zero_nu_b.jl.
# It uses that file's zero-nu-b parameter factory, model-validity gates, CSV
# helpers, and plotting defaults. The source-requested T=105 sweep is resolved
# at T=78, with the T=77 reference and K=76 cutoff supported by the zero case.

const NB19_SWEEP_SOURCE_REQUESTED_T = 105
const NB19_SWEEP_T_max = 78
const NB19_SWEEP_REFERENCE_T = 77
const NB19_SWEEP_BOUND_CUTOFF = 76
const NB19_SWEEP_ENDPOINT_GUARD = NB19_SWEEP_T_max - NB19_SWEEP_BOUND_CUTOFF
const NB19_SWEEP_HORIZONS = [30, 45, 60, 70, 75, 76, 77, 78]
const NB19_SWEEP_BRANCH_ITER_SCHEDULE = parse.(Int, strip.(split(get(
    ENV, "NB03_ZERO_NU_B_SWEEP_BRANCH_ITER_SCHEDULE", "60",
), ",")))
...
const NB19_SWEEP_MODE = Symbol(lowercase(get(
    ENV, "NB03_ZERO_NU_B_SWEEP_MODE", "full",
)))
NB19_SWEEP_MODE in (:full, :smoke) ||
    error("NB03_ZERO_NU_B_SWEEP_MODE must be full or smoke")
...
const NB19_PI_SWEEP_GRID = NB19_SWEEP_MODE == :smoke ? [0.75] :
    [0.70, 0.75, 0.80, 0.90, 0.95, 0.97, 0.99, 0.995]
const NB19_GAMMA_SWEEP_GRID = NB19_SWEEP_MODE == :smoke ? [0.25] :
...
println("Source-requested horizon:       ", NB19_SWEEP_SOURCE_REQUESTED_T)
println("Resolved sweep horizon T_max:   ", NB19_SWEEP_T_max)
...
println("Sweep mode:                      ", NB19_SWEEP_MODE)
...
function nb19_solve_sweep_case(pi_case::Float64, gamma_case::Float64;
    ...
    for T in NB19_SWEEP_HORIZONS
        ...
        for branch_iters in NB19_SWEEP_BRANCH_ITER_SCHEDULE
            ...
            try
                ...
                push!(attempt_rows, Dict{String,Any}(
                    ...
                    "pi"=>pi_case,
                    "gamma"=>gamma_case,
                    "source_requested_T_max"=>NB19_SWEEP_SOURCE_REQUESTED_T,
                    "T_max"=>T,
        ...
        if solved_here === nothing
            ...
            return (;
                ...
                accounting=nothing, absorbing=nothing, validity=nothing,
    ...
    absorbing = nb19_absorbing_normalization_summary(final_result)
    validity = nb19_hard_validity(final_result, accounting, absorbing)
    if !validity.valid
        failed_gates = String[]
        for field in (
            ...
            :phi_interior, :extended_bgp_valid, :normalization_valid,
            :exponent_ordering,
        )
        ...
        return (;
            ...
            accounting, absorbing, validity, attempt_rows, solved_by_T,
        )
    end
    ...
    return (;
        ...
        sweep_kind, sweep_value, label, pi_case, gamma_case,
        case_id=nb19_sweep_case_id(pi_case, gamma_case),
        result=final_result, reference_result, accounting, absorbing, validity,
        attempt_rows, solved_by_T,
    )
end
...
nb19_sweep_attempt_columns = [
    ...
    "source_requested_T_max", "T_max", "branch_iters", "warm_start_T",
    "seed_kind", "status", "elapsed_sec",
    "branch_converged", "max_u_residual", "max_bgp_residual",
    "residual_safe", "error",
]
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "sweep_solve_summary_T78.csv"),
...
function nb19_sweep_bound_data(case)
    ...
    T == NB19_SWEEP_T_max || error("Sweep bound requires resolved T_max=78")
    length(reference.u_path) == NB19_SWEEP_REFERENCE_T ||
        error("Sweep bound requires a T=77 reference")
    ...
    zero_exponents_ok = p.ν_b == 0.0 && p.ξ_W == 0.0
    zero_exponents_ok || error("Zero-case sweep bounds require nu_b=xi_W=0")
    @assert all(b -> b.ν_b_eff == 0.0, result.bgp_seq_extended) "nu_b_eff must remain fixed at zero"
    # The primitive zero exponent is the future-uniform bound; it is not a
    # switch-date state and no common-growth equation is part of the solve.
    nu_b_bound = 0.0
    ...
    finite_envelope_diagnostics_pass = finite_prefix_nonnegative &&
        ...
        prefix_stable && lambda_observed_within_bound && zero_exponents_ok &&
        nu_b_ordering && analytic_growth_floor_ok && denominators_ok &&
        isfinite(Abar_K) && Abar_K >= 0 &&
...
nb19_sweep_path_columns = [
    "sweep_kind", "sweep_value", "case_id", "label", "pi", "gamma",
    "source_requested_T_max", "T_max", "t", "model_t", "bound_status",
    "bubble_share_upper",
...
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "sweep_pi_five_quantities_T78.csv"),
    nb19_sweep_path_rows(pi_sweep_results), nb19_sweep_path_columns,
)
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "sweep_gamma_five_quantities_T78.csv"),
...
function nb19_sweep_summary_rows(cases)
    ...
    for case in cases
        ...
        push!(rows, Dict{String,Any}(
            ...
            "bound_status"=>solved ? s.bound_status : "unavailable_no_T78_solution",
            "phi_interior"=>solved ? case.validity.phi_interior : false,
            "phi_lower_slack"=>solved ? case.validity.phi_lower_slack : missing,
            "phi_upper_slack"=>solved ? case.validity.phi_upper_slack : missing,
            "absorbing_normalization_valid"=>
                solved ? case.validity.normalization_valid : false,
            "absorbing_policy_invariance_error"=>
                solved ? case.absorbing.policy_invariance_error : missing,
            "absorbing_aggregate_invariance_error"=>
                solved ? case.absorbing.aggregate_invariance_error : missing,
            "absorbing_per_variety_scaling_error"=>
                solved ? case.absorbing.per_variety_scaling_error : missing,
            "zero_exponent_compatibility_error"=>
                solved ? case.absorbing.exponent_compatibility_error : missing,
            "automatic_growth_identity_error"=>
                solved ? case.absorbing.automatic_growth_identity_error : missing,
            ...
            "zero_exponents_ok"=>solved ? s.zero_exponents_ok : false,
...
nb19_sweep_summary_columns = [
    "sweep_kind", "sweep_value", "case_id", "label", "pi", "gamma",
    "source_requested_T_max", "T_max", "solve_status", "failure_T",
    "failure_reason", "hard_valid",
    "max_u_residual", "max_bgp_residual",
    "bound_status", "phi_interior", "phi_lower_slack", "phi_upper_slack",
    "absorbing_normalization_valid", "absorbing_policy_invariance_error",
    "absorbing_aggregate_invariance_error", "absorbing_per_variety_scaling_error",
    "zero_exponent_compatibility_error", "automatic_growth_identity_error",
    "finite_envelope_diagnostics_pass",
    ...
    "analytic_growth_floor_ok", "zero_exponents_ok", "tail_dividend_envelope_ok",
...
function nb19_plot_five_quantity_sweep(results;
    ...
    if !isempty(unavailable)
        short_reason(c) = c.status == :failed_hard_gate ?
            "hard-validity gate" : "no residual-safe T=78 path"
        unavailable_text = "Unavailable at resolved Tmax=78:\n" *
...
pi_sweep_five_quantity_fig = nb19_plot_five_quantity_sweep(
    pi_sweep_results;
    sweep_title="AHP-calibrated two-country sweep over π, Tmax=78",
    filename="sweep_pi_five_quantities_T78.png",
    unavailable=pi_sweep_unavailable,
)
gamma_sweep_five_quantity_fig = nb19_plot_five_quantity_sweep(
    gamma_sweep_results;
    sweep_title="AHP-calibrated two-country sweep over γ, Tmax=78",
    filename="sweep_gamma_five_quantities_T78.png",
...
nb19_sweep_nfa_path_columns = [
    "sweep_kind", "sweep_value", "case_id", "label", "pi", "gamma",
    "source_requested_T_max", "T_max", "t", "model_t", "rebased_nfa_current_Y",
...
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "sweep_pi_nfa_components_T78.csv"),
    nb19_sweep_nfa_path_rows(pi_sweep_results),
    nb19_sweep_nfa_path_columns,
)
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "sweep_gamma_nfa_components_T78.csv"),
...
pi_sweep_nfa_figure = nb19_plot_sweep_nfa_decomposition(
    pi_sweep_results;
    sweep_title="AHP-calibrated U.S. NFA decomposition: π sweep, Tmax=78",
    filename="sweep_pi_nfa_12_panels_T78.png",
    unavailable=pi_sweep_unavailable,
)
gamma_sweep_nfa_figure = nb19_plot_sweep_nfa_decomposition(
    gamma_sweep_results;
    sweep_title="AHP-calibrated U.S. NFA decomposition: γ sweep, Tmax=78",
    filename="sweep_gamma_nfa_12_panels_T78.png",
    unavailable=gamma_sweep_unavailable,
)
...
println("\nInterpretation: lower bounds are not certified at resolved T_max=78; see")
println("sweep_case_summary_T78.csv for the HKT-entry and future-envelope qualifications.")
