#= ================================================================
   TwoCountryProductionOLG.jl  –  Common-growth-by-ξ_W types & helpers for the
   ...
   - The US absorbing exponent ν_b is fixed. At each possible switch state,
       the selected absorbing equilibrium recalibrates the local RoW exponent
       ξ_W so that post-switch US and RoW growth rates are equal.
   ================================================================ =#
...
# 1. Parameters
#
# The DEFAULTS reproduce the single-country numerical example of
# Hirano-Kishi-Toda (2025, arXiv:2501.08215, §4.2), mapped onto the
# two-country structure (RoW mirrors HKT's balanced-growth regime):
#   β=1/2, α=1/2, ρ=2, ϑ(θ)=1/2, A_X·H=10, A_L·L=1,
#   ξ_u=0.7, ν_u(=λ_u)=0.2, ν_b=ξ_W(=ξb=λb)=0.1, a·H=0.2, N_0=1.
...
#
# common_world_growth=true by default: the two-country bubble theorem
# (V4_3 thm_prod_sufficient) is proved under the equilibrium-selection
# condition ass_absorbing_stationary_equilibrium_selection, which imposes
# G_b=G_W across countries. HKT's literal ν_b=0.1 is a single-country
# primitive with no such constraint, so on the two-country BGP it gives
# G_b≠G_W (≈0.13%/period gap). In this isolated variant,
# calibrate_common_growth keeps ν_b fixed and recalibrates a local ξ_W at
# every possible switch state to enforce G_b=G_W. The all-u branch continues
# to use the baseline ξ_W primitive. Set false to solve with both exponents
# fixed and without the common-growth selection condition.
# ─────────────────────────────────────────────────────────────────
...
@with_kw struct ProductionParams
    ...
    ν_b::Float64       = 0.10          # fixed absorbing-regime exponent (HKT ξb=λb=0.1; was 0.30)
    ...
    ξ_u::Float64      = 0.70           # HKT ξu=0.7 (was 1.20); ξ_u > ν_u > ν_b
    ...
    ξ_W::Float64      = 0.10           # all-u primitive and initial seed for post-switch recalibration
    ...
    common_world_growth::Bool = true   # enforce G_b=G_W by recalibrating local post-switch ξ_W while holding ν_b fixed
...
function validate_params(p::ProductionParams)
    ...
    # V4_3 eq. psi_US_def (line 942):  ρ_US > 1,  ξ_u > ν_u > ν_b ≥ 0,
    #                                  ψ_US := (ξ_u - ν_u)(ρ_US - 1) > 0.
    @assert p.ν_u > p.ν_b                             "V4_3 psi_US_def: ν_u > ν_b"
...
struct BGPResult
    ...
    ν_b_eff::Float64                         # compatibility field; always the fixed primitive params.ν_b
    ξ_W_eff::Float64                         # local RoW exponent used for this selected BGP
...
function _zero_bgp_result()
    BGPResult(0.,0.,0.,0.,0.,0.,0.,0.,             # 8 policy slots
              ...
              1.,1.,0.,0.,                         # 2 growth-factor slots + ν_b_eff + ξ_W_eff
              NaN,                                 # Psi
...
#    Holds ν_b fixed and adjusts the local post-switch ξ_W such that
#    G_N_US^ν_b == G_N_W^ξ_W in the selected absorbing equilibrium.
#    Solve the scalar common-growth log gap with a safeguarded bracketed root
#    method. Every function evaluation solves the seven-equation BGP while
#    warm-starting from the nearest point already on the selected branch.
# ─────────────────────────────────────────────────────────────────

_bgp_actual_seed(b::BGPResult) =
    (b.φ_US, b.φ_W, b.ω, b.θ_US_star, b.ω_star, b.R_f, b.R_f_W)

function _common_growth_root_node(p::ProductionParams, N_US, N_W, ξ_W;
                                  x0_actual=nothing)
    p_node = ProductionParams(p; ξ_W=max(0.0, Float64(ξ_W)))
    bgp = solve_bgp_at(p_node, N_US, N_W; x0_actual=x0_actual)
    valid = bgp.converged && isfinite(bgp.residual_norm) && bgp.residual_norm < 1e-6 &&
            isfinite(bgp.G_N_US) && bgp.G_N_US > 0 &&
            isfinite(bgp.G_N_W) && bgp.G_N_W > 0
    gap = valid ? p.ν_b * log(bgp.G_N_US) - bgp.ξ_W_eff * log(bgp.G_N_W) : NaN
    return (params=p_node, bgp=bgp, gap=gap, valid=valid && isfinite(gap))
end

_common_growth_sign_change(a, b) =
    a == 0 || b == 0 || signbit(a) != signbit(b)

"""
Find the local post-switch ξ_W such that common-world-growth holds at the BGP
evaluated at (N_US, N_W).

Because φ_b and φ_W are themselves endogenous, this is a scalar root problem:
    G_b = G_N_US(φ_b)^ν_b
    G_W = G_N_W(φ_W)^ξ_W
    h(ξ_W) = ν_b*log(G_N_US(φ_b(ξ_W)))
             - ξ_W*log(G_N_W(φ_W(ξ_W))) = 0.

The supplied `p.ν_b` is never changed. Returns the locally calibrated ξ_W
and the corresponding BGPResult.
"""
function calibrate_common_growth(p::ProductionParams, N_US, N_W;
                                 max_iter::Int=240, tol::Float64=1e-8,
                                 verbose::Bool=false, x0_actual=nothing)
    seed = _common_growth_root_node(
        p, N_US, N_W, p.ξ_W; x0_actual=x0_actual)
    if !seed.valid
        verbose && @warn "Initial BGP is invalid; stopping common-growth calibration" residual=seed.bgp.residual_norm
        return (params=seed.params, bgp=seed.bgp)
    end
    abs(seed.gap) ≤ tol && return (params=seed.params, bgp=seed.bgp)

    # The fixed-point rearrangement supplies a scale-aware first bracket probe,
    # but it is not iterated. The actual solve below targets h(ξ_W)=0 directly.
    if seed.bgp.G_N_W ≤ 1.0 + 1e-12
        verbose && @warn "G_N_W too close to 1; stopping common-growth calibration"
        return (params=seed.params, bgp=seed.bgp)
    end
    ξ_predict = p.ν_b * log(seed.bgp.G_N_US) / log(seed.bgp.G_N_W)
    if !isfinite(ξ_predict) || ξ_predict < 0
        verbose && @warn "Invalid common-growth xi_W bracket predictor" ξ_predict
        return (params=seed.params, bgp=seed.bgp)
    end

    step = max(abs(ξ_predict - p.ξ_W), 0.05 * max(1.0, p.ξ_W), 1e-4)
    ξ_probe = max(0.0, ξ_predict)
    if abs(ξ_probe - p.ξ_W) ≤ 1e-12 * max(1.0, p.ξ_W)
        ξ_probe = seed.gap > 0 ? p.ξ_W + step : max(0.0, p.ξ_W - step)
    end
    probe = _common_growth_root_node(
        p, N_US, N_W, ξ_probe; x0_actual=_bgp_actual_seed(seed.bgp))
    if !probe.valid
        verbose && @warn "Common-growth bracket probe left the valid BGP branch" ξ_probe
        return (params=seed.params, bgp=seed.bgp)
    end
    abs(probe.gap) ≤ tol && return (params=probe.params, bgp=probe.bgp)

    best = abs(probe.gap) < abs(seed.gap) ? probe : seed
    left = seed
    right = probe
    bracketed = _common_growth_sign_change(seed.gap, probe.gap)
    if bracketed && left.params.ξ_W > right.params.ξ_W
        left, right = right, left
    end

    # If the predictor did not cross the root, expand only in the direction
    # implied by the seed gap. This selects the first continuation root reached
    # from the previous period's effective xi_W.
    if !bracketed && seed.gap > 0
        left, right = seed, probe
        if left.params.ξ_W > right.params.ξ_W
            left, right = right, left
        end
        for _ in 1:min(max_iter, 48)
            step *= 2
            ξ_next = right.params.ξ_W + step
            candidate = _common_growth_root_node(
                p, N_US, N_W, ξ_next; x0_actual=_bgp_actual_seed(right.bgp))
            candidate.valid || break
            abs(candidate.gap) < abs(best.gap) && (best = candidate)
            abs(candidate.gap) ≤ tol && return (params=candidate.params, bgp=candidate.bgp)
            if _common_growth_sign_change(right.gap, candidate.gap)
                left, right = right, candidate
                bracketed = true
                break
            end
            right = candidate
    ...
    elseif !bracketed
        left, right = probe, seed
        if left.params.ξ_W > right.params.ξ_W
            left, right = right, left
        end
        for _ in 1:min(max_iter, 48)
            step *= 2
            ξ_next = max(0.0, left.params.ξ_W - step)
            candidate = _common_growth_root_node(
                p, N_US, N_W, ξ_next; x0_actual=_bgp_actual_seed(left.bgp))
            candidate.valid || break
            abs(candidate.gap) < abs(best.gap) && (best = candidate)
            abs(candidate.gap) ≤ tol && return (params=candidate.params, bgp=candidate.bgp)
            if _common_growth_sign_change(candidate.gap, left.gap)
                left, right = candidate, left
                bracketed = true
                break
            end
            left = candidate
    ...
    if !bracketed
        verbose && @warn "Could not bracket the common-growth xi_W root" best_gap=best.gap
        return (params=best.params, bgp=best.bgp)
    end

    # Safeguarded secant iterations. Three consecutive updates to the same
    # bracket endpoint trigger a bisection step, preventing regula-falsi stalls.
    last_side = 0
    repeated_side = 0
    for it in 1:max_iter
        ξ_left = left.params.ξ_W
        ξ_right = right.params.ξ_W
        width = ξ_right - ξ_left
        ξ_trial = ξ_right - right.gap * width / (right.gap - left.gap)
        if !isfinite(ξ_trial) || ξ_trial ≤ ξ_left || ξ_trial ≥ ξ_right ||
           repeated_side ≥ 3
            ξ_trial = (ξ_left + ξ_right) / 2
            repeated_side = 0
        end

        nearest = abs(ξ_trial - ξ_left) ≤ abs(ξ_right - ξ_trial) ? left : right
        trial = _common_growth_root_node(
            p, N_US, N_W, ξ_trial; x0_actual=_bgp_actual_seed(nearest.bgp))
        if !trial.valid
            ξ_trial = (ξ_left + ξ_right) / 2
            nearest = abs(ξ_trial - ξ_left) ≤ abs(ξ_right - ξ_trial) ? left : right
            trial = _common_growth_root_node(
        ...
        trial.valid || break
        abs(trial.gap) < abs(best.gap) && (best = trial)

        verbose && @printf(
            "  root %2d: xi_W = %.10f, log gap = %.2e, bracket = %.2e\n",
            it, trial.params.ξ_W, trial.gap, width)
        if abs(trial.gap) ≤ tol
            return (params=trial.params, bgp=trial.bgp)
        end

        side = _common_growth_sign_change(left.gap, trial.gap) ? 1 : -1
        if side == 1
            right = trial
        else
            left = trial
        end
        repeated_side = side == last_side ? repeated_side + 1 : 1
        last_side = side
        if right.params.ξ_W - left.params.ξ_W ≤
           tol * max(1.0, abs(trial.params.ξ_W))
            break
        end
    end

    verbose && @warn "Common-growth scalar root did not reach tolerance" best_gap=best.gap
    return (params=best.params, bgp=best.bgp)
end
...
When `common_world_growth=true`, this keeps `ν_b` fixed and recalibrates the
local RoW exponent `ξ_W` for this specific switch state so that the selected
post-switch BGP satisfies `G_b = G_W`. The returned `BGPResult` stores it in
`ξ_W_eff`; the all-u branch continues to use the baseline `p.ξ_W`.
"""
function solve_selected_bgp_at(p::ProductionParams, N_US, N_W; x0_actual=nothing,
                               verbose::Bool=false)
    if p.common_world_growth
        cal = calibrate_common_growth(p, N_US, N_W; verbose=verbose,
                                      x0_actual=x0_actual)
        return cal.bgp
    else
...
function _bgp_continuation_seed(p::ProductionParams, previous::BGPResult)
    return p.common_world_growth ? ProductionParams(p; ξ_W=previous.ξ_W_eff) : p
end
...
was solved at, so the identity `ν_b·log G_N_US == ξ_W_eff·log G_N_W` holds for
...
function _bgp_state_residual_norm(p::ProductionParams, result::BGPResult, N_US, N_W)
    ...
    all(isfinite, (result.φ_US, result.φ_W, result.ω, result.θ_US_star,
                   ...
                   result.ν_b_eff, result.ξ_W_eff)) || return Inf
    result.ν_b_eff == p.ν_b || return Inf
    p_eff = p.common_world_growth ? ProductionParams(p; ξ_W=result.ξ_W_eff) : p
...
function _usable_bgp_continuation(p::ProductionParams, result::BGPResult, N_US, N_W)
    common_growth_ok = !p.common_world_growth ||
        (isfinite(result.G_N_US) && result.G_N_US > 0 &&
         ...
         isfinite(result.ν_b_eff) && result.ν_b_eff == p.ν_b &&
         isfinite(result.ξ_W_eff) &&
         abs(p.ν_b * log(result.G_N_US) -
             result.ξ_W_eff * log(result.G_N_W)) <= 1e-7)
    stored_ok = result.converged && isfinite(result.residual_norm) &&
           ...
           all(isfinite, (result.φ_US, result.φ_W, result.ω,
                          ...
                          result.R_f, result.R_f_W,
                          result.ν_b_eff, result.ξ_W_eff))
...
function u_residual!(F, x_c, p::ProductionParams,
    ...
    # Use the selected switch-branch BGP objects as stored. Under
    # common_world_growth, each BGP may have a state-specific ξ_W_eff, so
    # recomputing RoW q,d with the global p.ξ_W would use the wrong equilibrium.
...
function compute_diagnostics(p::ProductionParams,
    ...
    # The headline certificate should include the theorem's numerical
    # prerequisites, not just the two fitted tail conditions.
    bubble = geom_1a && geom_1b && psi_ok && equity_weights_ok &&
...
2. (Optional) hold ν_b fixed and calibrate local post-switch ξ_W for
   common world growth `G_b = G_W`
   (V4_3 ass_absorbing_stationary_equilibrium_selection, eq. 980–983).
3. Solve absorbing BGP at the initial state (N_US,0, N_W,0) via the
...
function run_production_simulation(p::ProductionParams=ProductionParams();
    ...
    p_use = p
    bgp0 = _zero_bgp_result()

    if p.common_world_growth
        verbose && println("Calibrating post-switch xi_W for common world growth (V4_3 G_b = G_W) with fixed nu_b...")
        cal = calibrate_common_growth(p, p.N_US_0, p.N_W_0; verbose=verbose)
        bgp0 = cal.bgp
        verbose && @printf("  Fixed nu_b = %.6f; initial selected xi_W = %.6f (seed %.6f)\n",
                           p.ν_b, bgp0.ξ_W_eff, p.ξ_W)
    else
...
#     ProductionParams DEFAULTS are now the HKT-matched calibration (see §1).
