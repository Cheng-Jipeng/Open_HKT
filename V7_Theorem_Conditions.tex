...
\begin{ass}[Positive lower bound on US total-saving returns]\label{ass_positive_total_saving_return}
\[
(1-\underline\theta)\underline R_t+\underline\theta\,\overline R_f>0,
\]
where $\underline\theta$ is given by \Cref{lem2}, and
$\underline R_t\equiv \min\{\underline R^u, \underline R^b_t\},\overline R_f$ are given by \Cref{lem_risky_return_bounds,lem_rf_upper_bound}.
\end{ass}

\begin{lem}[Equilibrium bounds on total-saving returns and old-age consumption]\label{lem_total_return_consumption_bounds}
Suppose
\Cref{ass_issuance_costs,ass_relative_dividend_bounds,ass_switch_us_fundamentals,ass_interior_equity_weights,ass_u_dividend_growth,ass_positive_total_saving_return}
hold.
...
$0<\underline R^u\le \overline R^u<\infty$, $\overline R^b<\infty$, $\overline R_f<\infty$, and $\underline\theta>-\infty$ together with $\underline R^b_t>0$ such that
\[
\underline R^u\le R_{p,t}^u\le \overline R^u,
\quad
\underline R^b_t\le R_{p,t}^z\le \overline R^b,
\quad
R_{f,t}\le \overline R_f,
\quad
	\theta_t\ge \underline{\theta},
...
R_{A,t}^b
\ge
(1-\theta_{t-1}^u)\underline R^b_t+\theta_{t-1}^u\overline R_f
\ge
\min\!\left\{
\underline R^b_t,\,
(1-\underline\theta)\underline R^b_t+\underline\theta\,\overline R_f
\right\}
\equiv
\underline R^b_{A,t}.
\end{align*}
By \Cref{ass_positive_total_saving_return}, $\underline R^u_{A}, \underline R^b_{A,t}>0$.
...
\begin{align*}
    ...
    \\
    0<\underline R^b_{A,t}\le R_{A,t}^b\le \overline R^b_A<\infty.
\end{align*}
...
For $C^b_t$, we first show that its upper bound is proportional to $e^b_t$:
\begin{align*}
    C^b_t 
    &=
    \beta e^u_{US,t-1} R^b_{A,t}
    \\& \le
    \beta e^u_{US,t-1}(1-\underline{\theta}) R^b_{p,t}
    \\& \le
    \beta e^u_{US,t-1}(1-\underline{\theta})\frac{(\bar q+\bar d)e^b_{US,t}}{\underline{q} e^u_{US,t-1}}
    \\&=
     \beta(1-\underline{\theta})\frac{(\bar q+\bar d)}{\underline{q}}e^b_{US,t}
\end{align*}
For the lower bound, we first consider the case with $\underline R^b_{A,t}=(1-\underline\theta)\underline R^b_t+\underline\theta\,\overline R_f$:
\begin{align*}
\begin{split}
		{C^b_t}
    &={\beta e^u_{US,t-1}R^b_{A,t}}
    \\& \geq{\beta e^u_{US,t-1}} \left[(1-\underline{\theta} )\underline{R}^b_t
    + \underline{\theta} \overline{R}_f  \right] 
     \\& ={\beta e^u_{US,t-1}} \left[(1-\underline{\theta} )\frac{\underline{q} e^b_{US,t}}{\overline{q}e^u_{US,t-1}}+ \underline{\theta} \overline{R}_f  \right]
      \\&=\beta(1-\underline{\theta} )\frac{\underline{q} }{\overline{q}}e^b_{US,t}
      + \beta \frac{e^u_{US,t-1}}{e^u_{US,t}} \frac{e^u_{US,t}}{e^b_{US,t}} e^b_{US,t}\underline{\theta} \overline{R}_f  
      \\& \geq \left[\beta(1-\underline{\theta} )\frac{\underline{q} }{\overline{q}}
      + \beta \frac{\bar \lambda_e}{\underline{g}_u} \underline{\theta} \overline{R}_f  \right] e^b_{US,t};
\end{split}
\end{align*}
if $\underline R^b_{A,t}=\underline R^b_t$, then
\[
		C^b_t
 		\geq{\beta e^u_{US,t-1}} \underline{R}^b_t
     ={\beta e^u_{US,t-1}} \frac{\underline{q} e^b_{US,t}}{\overline{q}e^u_{US,t-1}}
      =\beta\frac{\underline{q} }{\overline{q}}e^b_{US,t}.
\]
...
\\ &
\underline\lambda^b_C\equiv \min\{\beta\frac{\underline{q} }{\overline{q}},\beta(1-\underline{\theta} )\frac{\underline{q} }{\overline{q}}
      + \beta \frac{\bar \lambda_e}{\underline{g}_u} \underline{\theta} \overline{R}_f \}>0,
\\ &
\overline\lambda^b_C\equiv  \beta(1-\underline{\theta})\frac{(\bar q+\bar d)}{\underline{q}}<\infty,
\end{align*}
...
Suppose
\Cref{ass_issuance_costs,ass_regime_switch,ass_balanced_growth,ass_switch_dependence,ass_relative_dividend_bounds,ass_switch_us_fundamentals,ass_interior_equity_weights,ass_u_dividend_growth,ass_positive_total_saving_return}
hold. 
...
\propto \frac{e^b_t}{(e^b_t)^\gamma}= (e^b_t)^{1-\gamma},
\qquad
\frac{Q^u_t}{(C^u_t)^\gamma} 
\propto \frac{e^u_t}{(e^u_t)^\gamma}= (e^u_t)^{1-\gamma},
\]
