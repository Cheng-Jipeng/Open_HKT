...
# b-state:  Q^b_t = β·λ_e·e^u_t,  D^b_t = λ_D·D^u_t  (Assumption 5 from two-country).
#
# Equilibrium is closed-form:
#   R^u_t = g_e_u·(β + d^u_t)/β      (u-continuation return)
#   R^b_t = g_e_u·(β·λ_e + λ_D·d^u_t)/β  (b-switch return)
#   C^b/C^u = R^b/R^u → λ_e  as d^u_t → 0
#
# Key finding: Condition 2b ≡ Σ(C^b/C^u)^{1-γ} → constant × T → diverges.
# The cross-country portfolio rebalancing present in the two-country model
# (ω^u ≈ 0.14 → ω^b ≈ 0.80) is the mechanism absent here that can make it converge.
...
@with_kw struct SCParams
    ...
    # Switch-date scaling (Assumption 5): e^b = λ_e·e^u,  D^b = λ_D·D^u
    λ_e::Float64 = 1.0           # (same default as two-country)
    λ_D::Float64 = 1.0           # (same default)
...
Stock prices (market clearing, trivial):
  Q_u = β·e     (u-path: all young savings = stock market cap)
  Q_b = β·λ_e·e (b-state at this switch date)
...
function solve_sc_u_path(p::SCParams, paths::SCPaths)
    ...
    for t in 1:T
        ...
        states[t] = SCPeriodState(t, e_t, D_t, d, Q_u, Q_b, R_u, R_b)
    end
    states
end
...
Condition 2a: Σ d_t = Σ D^u_t / e^u_t < ∞
  → converges iff g_D_u < g_e_u  (which holds with default params)

Condition 2b: Σ (C^b_t / C^u_t)^{1-γ} < ∞
  → C^b/C^u = R_b/R_u = (β·λ_e + λ_D·d_t)/(β + d_t) → λ_e as t→∞
  → terms → (λ_e)^{1-γ}  (constant) → sum grows linearly → diverges
...
function run_sc_simulation(p::SCParams = SCParams(); verbose::Bool = true)
    ...
    verbose && @printf("  Switch scaling: λ_e=%.3f  λ_D=%.3f\n", p.λ_e, p.λ_D)
    ...
    if verbose
        ...
        @printf("  (C^b/C^u)^{1-γ} asymptote: %.6f  (= λ_e^{1-γ} = %.6f)\n",
                diag.cond_2b_terms[end], p.λ_e^(1 - p.γ))
        ...
        println("  Condition 2a converges iff g_D_u < g_e_u: dividends shrink")
        println("  relative to endowment so the 'fundamental' share → 0.")
        println()
        println("  Condition 2b requires (C^b/C^u)^{1-γ} → 0.  In the single-")
        println("  country model C^b/C^u → λ_e (constant), so terms are constant")
        println("  and the sum grows linearly — condition 2b always diverges.")
        println()
        println("  The two-country model adds cross-country portfolio rebalancing")
        println("  (ω^u ≈ 0.14 → ω^b ≈ 0.80) which can depress Q^u_US relative")
        println("  to Q^b_US, potentially driving C^b/C^u toward 0 and making")
        println("  condition 2b converge.")
    end
