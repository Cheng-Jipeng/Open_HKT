...
# G_b≠G_W (≈0.13%/period gap). With the flag on, calibrate_common_growth
# recalibrates ν_b (0.1 seed → ≈0.082) at runtime to enforce G_b=G_W;
# all other parameters stay HKT-literal. Set false to keep ν_b=0.1 exactly.
# ─────────────────────────────────────────────────────────────────
...
@with_kw struct ProductionParams
    ...
    ν_b::Float64       = 0.10          # absorbing-regime exponent (HKT ξb=λb=0.1; was 0.30)
    ...
    ξ_W::Float64      = 0.10           # HKT balanced exponent ξb=λb=0.1 (was 0.30)
    ...
    common_world_growth::Bool = true   # enforce G_b=G_W (V4_3 ass_absorbing_stationary_equilibrium_selection); recalibrates ν_b 0.1→≈0.082
...
struct BGPResult
    ...
    ν_b_eff::Float64                         # local absorbing exponent used for this selected BGP
...
function calibrate_common_growth(p::ProductionParams, N_US, N_W;
    ...
    for it in 1:max_iter
        ...
        bgp = solve_bgp_at(p_cur, N_US, N_W;
                           x0_actual=(bgp.φ_US, bgp.φ_W, bgp.ω, bgp.θ_US_star, bgp.ω_star,
                                       bgp.R_f, bgp.R_f_W))
...
When `common_world_growth=true`, this recalibrates the local absorbing exponent
`ν_b` for this specific switch state so that the selected post-switch BGP
satisfies `G_b = G_W`. The returned `BGPResult` stores the local exponent in
`ν_b_eff`; the all-u branch continues to use the baseline parameters.
"""
...
function u_residual!(F, x_c, p::ProductionParams,
    ...
    # common_world_growth, each BGP may have a state-specific ν_b_eff, so
    # recomputing q,d with the global p.ν_b would use the wrong equilibrium.
...
2. (Optional) calibrate ν_b for common world growth `G_b = G_W`
...
function run_production_simulation(p::ProductionParams=ProductionParams();
    ...
    if p.common_world_growth
        verbose && println("Calibrating ν_b for common world growth (V4_3 G_b = G_W)...")
