...
function nb19_find_project_dir(start_dir=pwd())
    ...
    while true
        for candidate in (dir, joinpath(dir, "Codes"))
            ...
            model = joinpath(candidate, "Two_country_production", "TwoCountryProductionOLG.jl")
...
const NB19_MODEL_FILE = joinpath(
    NB19_PROJECT_DIR, "Two_country_production", "TwoCountryProductionOLG.jl",
)
...
const NB19_OUTDIR = get(
    ENV, "NB19_OUTDIR",
    joinpath(
        NB19_PROJECT_DIR, "Two_country_production", "outputs_v43",
        "ahp_pattern_T110_replication",
    ),
)
...
best_validity.valid || error("The T=110 continuation failed a Notebook 15/17 hard gate")
...
nb19_max_first_window_error <= NB19_FIRST_WINDOW_MATCH_TOL || error(
    "T=110 first-window replication error $(nb19_max_first_window_error) exceeds tolerance",
)
...
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "best_common_growth_diagnostics.csv"),
...
p_ahp = plot(
    ...
    title="AHP decomposition at T=110: t=1:$(e) is the evidence window",
...
p_ratio = plot(
    xlabel="period t", ylabel="fraction of current U.S. output",
    title="NFA/current Y (full T=110 tail)", legend=:outerbottom,
)
...
p_nu = plot(
    switch_t, nu_eff; color=:purple, lw=2.6, label="nu_b_eff",
...
savefig(
    fig_common_growth,
    joinpath(NB19_OUTDIR, "best_common_growth_diagnostics.png"),
)

println("\n" * repeat("=", 88))
println("NOTEBOOK 19 — NOTEBOOK 15 AHP FIGURES ON THE T=110 CONTINUATION")
println(repeat("=", 88))
