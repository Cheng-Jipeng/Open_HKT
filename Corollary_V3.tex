...
$\eta\,\Upsilon(\theta_t)$ captures a convex cost in the present-value bond share $\theta_t$ (defined below),
disciplines large bond positions, and preserves the saving separation.

\paragraph{Stock pricing (US effective SDF).}
The FOCs for equity positions imply, for $i\in\{US,W\}$,
\begin{align}\label{US_AP_bond_cost}
Q_{i,t}
=
E_t\!\left[\tilde M^{\,i}_{t,t+1}\bigl(Q_{i,t+1}+D_{i,t+1}\bigr)\right],
\end{align}
with\footnote{We defer the discussion of how to guarantee the effective pricing kernels are well-behaved to
\Cref{sec:existence_of_bubbles}.}
\begin{align*}
\tilde M^{\,US}_{t,t+1}=\frac{M_{t,t+1}}{1+\phi_t(1-\omega_t)},
\qquad
\tilde M^{\,W}_{t,t+1}=\frac{M_{t,t+1}}{1-\phi_t\omega_t},\\
M_{t,t+1}
=\frac{\beta}{1-\beta}\,
\frac{C_{y,t}\,C_{o,t+1}^{-\gamma}}{E_t\!\left[C_{o,t+1}^{1-\gamma}\right]},
\qquad
\phi_t \equiv \frac{\kappa C_{y,t}}{(1-\beta)S_t}\,(\omega_t-\bar\omega).
\end{align*}
...
\paragraph{Portfolio FOCs.}
Define the normalized portfolio kernel
\[
\mathcal M_{t,t+1} \equiv \frac{R_{A,t+1}^{-\gamma}}{E_t\!\left[R_{A,t+1}^{1-\gamma}\right]}.
\]
The FOC for $\omega_t$ implies the (scaled) return wedge condition
\begin{align}\label{US_omega_FOC_bond_cost}
\beta(1-\theta_t)\,E_t\!\left[\mathcal M_{t,t+1}\bigl(R_{US,t+1}-R_{W,t+1}\bigr)\right]
=\kappa(\omega_t-\bar\omega).
\end{align}
The FOC for $\theta_t$ (bond vs equity share) is
...
With some abuse of notation, in this section we denote the \emph{effective} pricing kernel that prices the US stock,
$\tilde M^{\,US}_{t,t+1}$ in \eqref{US_AP_bond_cost}, by $\mathcal M_{t,t+1}$. That is,
\[
\mathcal M_{t,t+1}\equiv \tilde M^{\,US}_{t,t+1}
=\frac{M_{t,t+1}}{1+\phi_t(1-\omega_t)}.
\]
Using the saving rule \eqref{US_saving_rule_bond_cost} and the fact that $C_{o,t+1}=A_tR_{A,t+1}$, the US \emph{undistorted}
SDF simplifies to
\[
M_{t,t+1}
=\frac{R_{A,t+1}^{-\gamma}}{E_t\!\left[R_{A,t+1}^{1-\gamma}\right]}
\equiv \mathcal K_{t,t+1},
\]
so the only additional distortion in the effective kernel $\mathcal M_{t,t+1}$ comes from the portfolio wedge.
Note that because the RoW utility includes $\chi\log(q_{US,t}B^*_{US,t+1})$, feasibility requires $B^*_{US,t+1}>0$.
With zero net supply of US bonds, $B_{US,t+1}+B^*_{US,t+1}=0$ implies $B_{US,t+1}<0$, hence
$b_t=q_{US,t}B_{US,t+1}<0$ and
\begin{equation}\label{theta_nonpos}
\theta_t=\frac{b_t}{A_t}\le 0
\quad\Rightarrow\quad
1-\theta_t\ge 1 .
\end{equation}
Below we provide sufficient conditions for well-behaved effective kernels.

\begin{lem}\label{lem1}
Let
\[
\Delta_t \equiv \phi_t(1-\omega_t),
\qquad\text{where}\qquad
\phi_t=\frac{\kappa C_{y,t}}{(1-\beta)S_t}\,(\omega_t-\bar\omega).
\]
If $\bar\omega < \frac{\beta}{\kappa}$,
then $1+\Delta_t>0$ for all $t$.
\end{lem}

Therefore, we will limit parameters so that $\frac{1}{2}<\bar\omega<\beta/\kappa$ from now on.\footnote{
	This may force $\kappa$ to be extremely small as $\kappa <2 \beta$ is implicitly imposed. 
	But also note that this is just a sufficient condition for $\mathcal{\tilde M}^{US}_{t,t+1}$ 
	to be well-behaved, and the actual safe range for $\kappa$ could be much larger.}
...
\Cref{ass1} implies that state $b$ is absorbing. We further assume that once state $b$ is reached, uncertainty is resolved,
...
If \Cref{ass1,ass2} hold, once state $b$ is reached, the US stock price equals its fundamental value:
$Q_{US,t}=V_{US,t}$.
\end{prop}
...
$\tau =\min \{t:z_t=b\}$. By \Cref{ass1}, $\tau$ follows a geometric distribution,
\[
...
Suppose \Cref{ass1,ass2} hold. Then there is no bubble if and only if
\[
...
Suppose Assumptions \ref{ass1}--\ref{ass3} hold. If $z_0=u$, then there is a bubble at $t=0$ if and only if
...
Finally, under \Cref{ass3}, the portfolio choices $(\omega_{t-1},\theta_{t-1})$ are $\mathcal F_{t-1}$-measurable and do not depend on $z_t$.
Thus the wedge term $1+\Delta_{t-1}=1+\phi_{t-1}(1-\omega_{t-1})$ is common across $z_t\in\{u,b\}$ and cancels in the ratio:
\[
\frac{\mathcal M_{t-1,t}^b}{\mathcal M_{t-1,t}^u}
=
...
in \Cref{thm_1} into exogenous variables along the $u$-history and at the switching date. We first impose mild
restrictions on relative country size and on the nature of regime switching.

\begin{ass}\label{ass4}
There exists $\overline\varepsilon_e<\infty$ such that
\[
\frac{e_{W,t}}{e_{US,t}^u}\le \overline\varepsilon_e
\qquad\text{for all }t\le \tau .
\]
\end{ass}

\begin{ass}\label{ass5}
For $x\in\{e_{US},D_{US},e_W,D_W\}$, $x_t^b=x_t^u$ whenever $z_{t-1}=u$ and $z_t=b$.
\end{ass}


We also assume that issuance-cost function $\Upsilon$
 satisfies standard regularity and Inada-type conditions.

\begin{ass}[Issuance costs]\label{ass6}
...
Suppose \Cref{ass6} holds and the bond-share FOC \eqref{US_theta_FOC_bond_cost} holds with $\eta>0$ and finite LHS,
i.e.,
\[
...
$\Upsilon'$ is strictly increasing and continuous on $(-\infty,1)$, and by \Cref{ass6} it is onto $\mathbb R$.
...
Suppose Assumptions \ref{ass1}--\ref{ass6} hold. If:

(i) $\omega_t$ is interior, i.e., there exists $\underline{\varepsilon}_\omega>0$ such that
$\omega_t\ge \underline{\varepsilon}_\omega$ for all $t\le\tau$;

(ii) the bond-share FOC \eqref{US_theta_FOC_bond_cost} holds with $\eta>0$ and finite LHS, i.e.,
$\left|\,\beta\,E_t\!\left[\mathcal M_{t,t+1}\bigl(R_{f,t}-R_{p,t+1}\bigr)\right]\right|\le \overline\xi$;

(iii) $R_{f,t}$ is uniformly bounded above on $t\le\tau$, i.e., $R_{f,t}\le \overline R_f<\infty$; and

(iv) $(D_{W,t}^z/e_{US,t}^z)\le \bar\delta_W$ for all $t$ and $z\in\{u,b\}$,
...
\begin{proof}
We show (a) \eqref{thm_cond_1a} $\Longleftrightarrow$ \eqref{thm_cond_2a} and (b) \eqref{thm_cond_1b}
$\Longleftrightarrow$ \eqref{thm_cond_2b}.

\medskip
\noindent\textbf{(a) Proof of \eqref{thm_cond_1a} $\Longleftrightarrow$ \eqref{thm_cond_2a}.}
By \eqref{Qsum_identity_mod_zerosupply} and \Cref{ass4},
\[
Q_{US,t}^u \le Q_{US,t}^u+Q_{W,t}^u
=\beta e_{US,t}^u+\frac{\beta+\chi}{1+\chi}e_{W,t}
\le \left(\beta+\frac{\beta+\chi}{1+\chi}\overline\varepsilon_e\right)e_{US,t}^u,
\qquad t\le\tau.
\]
...
$Q_{US,t}^u\ge \underline\varepsilon_\omega\,\beta\,e_{US,t}^u$ for all $t\le\tau$.
Hence there exist constants $0<\underline q\le \bar q<\infty$ such that
$\underline q\,e_{US,t}^u\le Q_{US,t}^u\le \bar q\,e_{US,t}^u$ for all $t\le\tau$, implying
$\sum_t D_t^u/Q_t^u<\infty$ if and only if $\sum_t D_t^u/e_{US,t}^u<\infty$.
...
Old-age consumption at date $t$ is
\[
C_t^z
=
(Q_{US,t}^z+D_{US,t}^z)n_{US,t-1}^u
+(Q_{W,t}^z+D_{W,t}^z)n_{W,t-1}^u
+B_{US,t}^u,
\qquad z\in\{u,b\}.
\]
By \Cref{ass4,ass5} and \eqref{Qsum_identity_mod_zerosupply}, there exists $\bar q<\infty$ such that
$0<Q_{US,t}^z\le Q_{US,t}^z+Q_{W,t}^z\le \bar q\,e_{US,t}^u$ for all $t\le\tau$ and $z\in\{u,b\}$.
The dividend bound in (iv) implies $0\le D_{W,t}^z\le \bar\delta_W e_{US,t}^u$. If \eqref{thm_cond_2a} holds, then
$\sup_t D_{US,t}^u/e_{US,t}^u<\infty$, and by \Cref{ass5} the same bound applies at the switching date, so
$0\le D_{US,t}^z\le \bar\delta_{US}e_{US,t}^u$ for some $\bar\delta_{US}<\infty$.

Next, the holdings satisfy
\[
n_{US,t-1}^u
=\frac{\omega_{t-1}^u S_{t-1}^u}{Q_{US,t-1}^u}
=\frac{\omega_{t-1}^u}{\omega_{t-1}^u+\omega_{t-1}^{*,u}\,S_{t-1}^{*,u}/S_{t-1}^u}.
\]
Using \eqref{theta_nonpos}, we have $S_{t-1}^u\ge \beta e_{US,t-1}^u$, while
$S_{t-1}^{*,u}\le Q_{US,t-1}^u+Q_{W,t-1}^u\le \bar q\,e_{US,t-1}^u$, hence
$S_{t-1}^{*,u}/S_{t-1}^u\le \bar q/\beta =:\overline\rho<\infty$. Together with $\omega_{t-1}^u\ge \underline\varepsilon_\omega$
and $\omega_{t-1}^{*,u}\le 1$, this yields $n_{US,t-1}^u\ge \underline n>0$ uniformly for $t\le\tau$.

Finally, by \Cref{lem2} we have $\theta_t\ge \underline\theta$ for all $t\le\tau$, and by \eqref{theta_nonpos} we also have
$\theta_t\le 0$. Using $B_{US,t}=b_{t-1}R_{f,t-1}=\theta_{t-1}A_{t-1}R_{f,t-1}$, $A_{t-1}=\beta e_{US,t-1}^u$, and
$R_{f,t-1}\le \overline R_f$ from (iii), we obtain a uniform lower bound
$B_{US,t}^u \ge \underline\theta\,\beta\,\overline R_f\,e_{US,t-1}^u$.
Combining this bound with the payoff bounds above and the holdings bounds yields constants
$0<\underline C\le \bar C<\infty$ such that for all $t\le\tau$ and $z\in\{u,b\}$,
\[
\underline C\,e_{US,t}^u \le C_t^z \le \bar C\,e_{US,t}^u.
\]
Therefore $(Q_t^b+D_t^b)/C_t^b$ and $C_t^u/Q_t^u$ are both bounded above and below by strictly positive constants, and so
is their product $m_t$. Hence \eqref{thm_cond_1b} holds if and only if \eqref{thm_cond_2b}.
\end{proof}
...
\appendix
\section{Proof of \Cref{lem1}}
Recall
\[
\Delta_t=\phi_t(1-\omega_t),\qquad 
\phi_t=\frac{\kappa C_{y,t}}{(1-\beta)S_t}(\omega_t-\bar\omega).
\]
Using the saving rule $C_{y,t}=(1-\beta)e_{US,t}$ and $A_t=\beta e_{US,t}$, we have
\[
\frac{C_{y,t}}{(1-\beta)}=e_{US,t}=\frac{A_t}{\beta}.
\]
Also $S_t=(1-\theta_t)A_t$, hence
\[
\frac{C_{y,t}}{(1-\beta)S_t}
=\frac{A_t/\beta}{(1-\theta_t)A_t}
=\frac{1}{\beta(1-\theta_t)}.
\]
Therefore
\[
\phi_t=\frac{\kappa}{\beta(1-\theta_t)}(\omega_t-\bar\omega),
\qquad
\Delta_t=\frac{\kappa}{\beta(1-\theta_t)}(\omega_t-\bar\omega)(1-\omega_t).
\]

Moreover, $\omega_t\in[0,1]$ implies $(1-\omega_t)\in[0,1]$ and
\[
(\omega_t-\bar\omega)(1-\omega_t)\ge -\bar\omega(1-\omega_t)\ge -\bar\omega.
\]
Combining the bounds yields
\[
\Delta_t
=\frac{\kappa}{\beta(1-\theta_t)}(\omega_t-\bar\omega)(1-\omega_t)
\ge \frac{\kappa}{\beta(1-\theta_t)}\cdot(-\bar\omega)
= -\frac{\kappa\bar\omega}{\beta(1-\theta_t)}.
\]
Hence
\[
1+\Delta_t \;\ge\; 1-\frac{\kappa\bar\omega}{\beta(1-\theta_t)} 
\Longleftarrow \bar \omega \leq \frac{\beta(1-\theta_t)}{\kappa}.
\]
If $\bar\omega \leq \beta/\kappa$, then the right-hand side is strictly positive because of 
$1-\theta_t > 1$, so
$1+\Delta_t>0$ for all $t$.
\medskip
