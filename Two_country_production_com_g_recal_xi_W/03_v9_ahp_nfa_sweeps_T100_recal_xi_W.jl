# Included after 03_v9_ahp_pattern_T110_replication_recal_xi_W.jl. This file
...
const NB19_NFA_SWEEP_INNER_ITERATIONS = parse(Int, get(
    ENV, "NB03_RECAL_XI_W_NFA_SWEEP_INNER_ITERATIONS",
    string(NB19_SWEEP_INNER_ITERATIONS),
))
@assert NB19_NFA_SWEEP_INNER_ITERATIONS > 0
...
println("Inner nonlinear-iteration cap: ", NB19_NFA_SWEEP_INNER_ITERATIONS,
        " (sweeps only)")
...
function nb19_nfa_sweep_params(T::Int, branch_iters::Int;
    ...
    return ProductionParams(
        ...
        iterations=NB19_NFA_SWEEP_INNER_ITERATIONS,
        branch_iters=branch_iters, do_global_polish=global_polish,
    )
end

nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "nfa_sweep_T100_numerical_budget.csv"),
    [Dict{String,Any}(
        "scope"=>"portfolio_weight_robustness_sweeps_only",
        "inner_nonlinear_iterations"=>NB19_NFA_SWEEP_INNER_ITERATIONS,
        "branch_iteration_schedule"=>join(NB19_NFA_SWEEP_BRANCH_ITER_SCHEDULE, ";"),
        "parameter_bridge_step"=>0.01,
        "T110_baseline_uses_model_default"=>true,
    )],
    ["scope", "inner_nonlinear_iterations", "branch_iteration_schedule",
     "parameter_bridge_step", "T110_baseline_uses_model_default"],
)
...
function nb19_nfa_plot_feasible(result, validity)
    ...
    return validity.model_residuals && validity.psi_interior &&
           ...
           validity.exact_common && validity.extended_bgp_valid &&
...
function nb19_nfa_case_summary_rows(cases)
    ...
    for case in cases
        ...
        push!(rows, Dict{String,Any}(
            ...
            "hard_valid_economic_feasibility"=>
...
nb19_nfa_case_summary_columns = [
    ...
    "hard_valid_economic_feasibility", "equity_interior_one_percent",
