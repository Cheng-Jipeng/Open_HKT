...
and continuation-branch growth and dividends.
...
\begin{ass}[Continuation-branch US dividends and one-step US growth]\label{ass_u_dividend_growth}
...
Let
\[
\overline d_{US}\equiv \max\{\overline\lambda_{D_{US}},\overline\lambda_{D^b_{US}}\},
\qquad
...
Then
\[
D_{US,t}^z\le \overline d_{US}\,e_{US,t}^u,
\qquad
D_{W,t}^z\le \overline d_W\,e_{US,t}^u,
\qquad
...
\Cref{ass_issuance_costs} holds. Then there exists a constant $\underline\theta>-\infty$ such that
\[
...
\[
R_{p,t+1}(\bar\omega) \ge \underline R >0,
\]
so this choice yields a finite lower bound
\[
...
\begin{ass}[Switch-date debt drag is dominated by risky payoffs]\label{ass_switch_debt_drag}
There exists a constant $\xi\in[0,1)$ such that, whenever $z_{t-1}=u$ and $z_t=b$,
\[
-\theta_{t-1}^u\bigl(R_{f,t-1}-R_{p,t}^b\bigr)\le \xi R_{p,t}^b.
\]
Equivalently,
\[
R_{A,t}^b\ge (1-\xi)R_{p,t}^b.
\]
\end{ass}

Assumption \ref{ass_switch_debt_drag} controls the debt-repayment drag on the switch branch directly. In the next lemma we keep a direct lower bound on the continuation-branch total-saving return $R_{A,t}^u$ as an explicit hypothesis.

\begin{lem}[Equilibrium bounds on total-saving returns and old-age consumption]\label{lem_total_return_consumption_bounds}
Suppose
\Cref{ass_issuance_costs,ass_relative_dividend_bounds,ass_switch_us_fundamentals,ass_interior_equity_weights,ass_u_dividend_growth,ass_switch_debt_drag}
hold, suppose dividends are nonnegative, and suppose there exists a constant $\underline R_A^u>0$ such that
\[
R_{A,t}^u\ge \underline R_A^u
\qquad\text{for all }t\le\tau.
\]
...
\begin{proof}
By \Cref{lem_risky_return_bounds,lem_rf_upper_bound,lem2}, there exist constants
$0<\underline R^u\le \overline R^u<\infty$, $\overline R^b<\infty$,
$\overline R_f<\infty$, and $\underline\theta>-\infty$ such that
\[
\underline R^u\le R_{p,t}^u\le \overline R^u,
\qquad
0<R_{p,t}^b\le \overline R^b,
\qquad
R_{f,t}\le \overline R_f,
\qquad
\theta_t\in[\underline\theta,0)
\]
for all $t\le\tau$.
For $z\in\{u,b\}$,
\[
...
\]
Since $\theta_{t-1}^u<0$ and $R_{f,t-1}>0$,
\[
R_{A,t}^z
\le
(1-\theta_{t-1}^u)R_{p,t}^z
\le
...
where
\[
\overline R_A^u\equiv (1-\underline\theta)\overline R^u,
\qquad
\overline R_A^b\equiv (1-\underline\theta)\overline R^b.
\]
The displayed continuation-branch hypothesis therefore gives
\[
0<\underline R_A^u\le R_{A,t}^u\le \overline R_A^u<\infty.
\]

On the switch branch, \Cref{ass_switch_debt_drag} implies
\[
R_{A,t}^b
=
R_{p,t}^b-\bigl[-\theta_{t-1}^u(R_{f,t-1}-R_{p,t}^b)\bigr]
\ge
(1-\xi)R_{p,t}^b
>
0.
...
Let
\[
\overline d\equiv \max\{\overline\lambda_{D_{US}},\overline\lambda_{D_W}\}.
\]
By \Cref{lem_stock_value_bounds}, nonnegative dividends, and \Cref{ass_u_dividend_growth}, for each $i\in\{US,W\}$,
\[
\underline q\,e_{US,t}^b
\le
Q_{i,t}^b+D_{i,t}^b
\le
(\overline q+\overline d)e_{US,t}^b,
\qquad
\underline q\,e_{US,t-1}^u
\le
Q_{i,t-1}^u
\le
\overline q\,e_{US,t-1}^u.
\]
Hence
\[
\frac{\underline q}{\overline q}\frac{e_{US,t}^b}{e_{US,t-1}^u}
\le
R_{i,t}^b
\le
\frac{\overline q+\overline d}{\underline q}\frac{e_{US,t}^b}{e_{US,t-1}^u},
\qquad i\in\{US,W\}.
\]
Because $R_{p,t}^b$ is a convex combination of $R_{US,t}^b$ and $R_{W,t}^b$, it follows that
\[
\frac{\underline q}{\overline q}\frac{e_{US,t}^b}{e_{US,t-1}^u}
\le
R_{p,t}^b
...
\frac{\overline q+\overline d}{\underline q}\frac{e_{US,t}^b}{e_{US,t-1}^u}.
\]
Combining the last display with the switch-debt-drag condition yields
\[
(1-\xi)\frac{\underline q}{\overline q}\frac{e_{US,t}^b}{e_{US,t-1}^u}
\le
R_{A,t}^b
\le
(1-\underline\theta)\frac{\overline q+\overline d}{\underline q}\frac{e_{US,t}^b}{e_{US,t-1}^u}.
\]
...
\]
\Cref{ass_u_dividend_growth} implies
\[
\frac{\beta\underline R_A^u}{\overline g_u}\,e_{US,t}^u
\le
C_t^u
\le
\frac{\beta\overline R_A^u}{\underline g_u}\,e_{US,t}^u,
\]
and the switch-branch bounds imply
\[
\beta(1-\xi)\frac{\underline q}{\overline q}e_{US,t}^b
\le
C_t^b
\le
\beta(1-\underline\theta)\frac{\overline q+\overline d}{\underline q}e_{US,t}^b.
\]
Hence one may take
\[
\underline\lambda^u_C\equiv \frac{\beta\underline R_A^u}{\overline g_u},
\qquad
\overline\lambda^u_C\equiv \frac{\beta\overline R_A^u}{\underline g_u},
\]
and
\[
\underline\lambda^b_C\equiv \beta(1-\xi)\frac{\underline q}{\overline q},
\qquad
\overline\lambda^b_C\equiv \beta(1-\underline\theta)\frac{\overline q+\overline d}{\underline q},
\]
...
Suppose
\Cref{ass_issuance_costs,ass_regime_switch,ass_balanced_growth,ass_switch_dependence,ass_relative_dividend_bounds,ass_switch_us_fundamentals,ass_interior_equity_weights,ass_u_dividend_growth,ass_switch_debt_drag}
hold, suppose dividends are nonnegative, and suppose there exists a constant $\underline R_A^u>0$ such that
\[
R_{A,t}^u\ge \underline R_A^u
\qquad\text{for all }t\le\tau.
\]
For $z\in\{u,b\}$, let $x_t^z$ denote the value of $x_t$ conditional on $z_0=\cdots=z_{t-1}=u$ and $z_t=z$.
...
By \Cref{lem_stock_value_bounds},
\[
\underline q\,e_{t}^u\le Q_t^u\le \overline q\,e_{t}^u,
\qquad
\underline q\,e_{t}^b\le Q_t^b\le \overline q\,e_{t}^b.
\]
By nonnegative dividends and \Cref{ass_u_dividend_growth}, if we set
\[
...
\overline q_b\,e_t^b.
\]
By \Cref{lem_total_return_consumption_bounds}, there exist constants
$0<\underline\lambda_C^z\le \overline\lambda_C^z<\infty$ such that
\[
\underline\lambda_C^z\,e_t^z
\le
C_t^z
\le
\overline\lambda_C^z\,e_t^z,
\qquad
z\in\{u,b\},\ \forall t\le\tau.
\]
Therefore,
\[
\frac{\underline q_b}{(\overline\lambda_C^b)^\gamma}(e_t^b)^{1-\gamma}
\le
\frac{Q_t^b+D_t^b}{(C_t^b)^\gamma}
\le
\frac{\overline q_b}{(\underline\lambda_C^b)^\gamma}(e_t^b)^{1-\gamma},
\]
and
\[
\frac{\underline q}{(\overline\lambda_C^u)^\gamma}(e_t^u)^{1-\gamma}
\le
\frac{Q_t^u}{(C_t^u)^\gamma}
\le
\frac{\overline q}{(\underline\lambda_C^u)^\gamma}(e_t^u)^{1-\gamma}.
\]
Hence there exist constants $0<c_1\le c_2<\infty$ such that
\[
c_1\left(\frac{e_t^b}{e_t^u}\right)^{1-\gamma}
\le
\frac{\frac{Q_t^b+D_t^b}{(C_t^b)^\gamma}}{\frac{Q_t^u}{(C_t^u)^\gamma}}
\le
c_2\left(\frac{e_t^b}{e_t^u}\right)^{1-\gamma}
\]
for all $t\le\tau$. Therefore
\[
\sum_{t=1}^\infty
\frac{\frac{Q_t^b+D_t^b}{(C_t^b)^\gamma}}{\frac{Q_t^u}{(C_t^u)^\gamma}}
<\infty
\quad\Longleftrightarrow\quad
\sum_{t=1}^\infty
\left(\frac{e_t^b}{e_t^u}\right)^{1-\gamma}
<\infty.
\]
