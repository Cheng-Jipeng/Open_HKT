# This file is included after 03_v9_ahp_pattern_T110_replication_recal_xi_W.jl.
# It uses that file's fixed-nu_b/recalibrated-xi_W specification, validity gates, CSV
...
const NB19_SWEEP_BRANCH_ITER_SCHEDULE = parse.(Int, strip.(split(get(
    ENV, "NB03_RECAL_XI_W_SWEEP_BRANCH_ITER_SCHEDULE", "60",
), ",")))
@assert !isempty(NB19_SWEEP_BRANCH_ITER_SCHEDULE) &&
        all(>(0), NB19_SWEEP_BRANCH_ITER_SCHEDULE)
# The source default of 4,000 inner nonlinear iterations caused one
# near-boundary sweep cell to exceed 7,200 seconds.  Bound only the robustness
# sweeps; the T=110 baseline continues to use the model default.
const NB19_SWEEP_INNER_ITERATIONS = parse(Int, get(
    ENV, "NB03_RECAL_XI_W_SWEEP_INNER_ITERATIONS", "100",
))
@assert NB19_SWEEP_INNER_ITERATIONS > 0
...
println("Inner nonlinear-iteration cap:  ", NB19_SWEEP_INNER_ITERATIONS,
        " (sweeps only; baseline remains at model default)")
...
function nb19_sweep_params(T::Int, branch_iters::Int;
    ...
    return ProductionParams(
        T_max=T, n_buffer=0, common_world_growth=true,
        ...
        iterations=NB19_SWEEP_INNER_ITERATIONS,
        branch_iters=branch_iters, do_global_polish=false,
    )
end

nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "sweep_numerical_budget.csv"),
    [Dict{String,Any}(
        "scope"=>"pi_gamma_robustness_sweeps_only",
        "inner_nonlinear_iterations"=>NB19_SWEEP_INNER_ITERATIONS,
        "branch_iteration_schedule"=>join(NB19_SWEEP_BRANCH_ITER_SCHEDULE, ";"),
        "reason"=>"uncapped 4000-iteration inner solve exceeded 7200-second notebook cell timeout",
        "T110_baseline_uses_model_default"=>true,
    )],
    ["scope", "inner_nonlinear_iterations", "branch_iteration_schedule",
     "reason", "T110_baseline_uses_model_default"],
)
...
function nb19_solve_sweep_case(pi_case::Float64, gamma_case::Float64;
    ...
    if !validity.valid
        ...
        for field in (
            ...
            :theta_feasible, :positive_foreign_claims, :positive_returns,
        ...
        gate_detail = validity.theta_feasible ? "" :
            @sprintf("; min theta_US_star=%.6f < %.3f",
                     theta_min, NB19_THETA_FEASIBILITY_LOWER)
        return (;
...
function nb19_sweep_bound_data(case)
    ...
    # The primitive nu_b=0.10 is global in this model, hence it is itself the
    # future-uniform US absorbing-exponent bound. Common growth is implemented
    # through the separate state-specific xi_W_eff sequence.
    nu_b_bound = p.ν_b
...
function nb19_sweep_summary_rows(cases)
    ...
    for case in cases
        ...
        push!(rows, Dict{String,Any}(
            ...
            "fixed_nu_b"=>solved ? case.result.params.ν_b : missing,
            "all_u_xi_W_seed"=>solved ? case.result.params.ξ_W : missing,
            "nu_b_eff_min"=>solved ? case.common.nu_b_eff_min : missing,
            "nu_b_eff_max"=>solved ? case.common.nu_b_eff_max : missing,
            "xi_W_eff_min"=>solved ? case.common.xi_W_eff_min : missing,
            "xi_W_eff_max"=>solved ? case.common.xi_W_eff_max : missing,
            "max_common_growth_log_gap"=>solved ? case.common.max_log_gap : missing,
            "exact_common_growth"=>solved ? case.validity.exact_common : false,
            "theta_strictly_feasible"=>solved ? case.validity.theta_feasible : false,
            "theta_one_percent_interior"=>solved ? case.validity.theta_interior : false,
...
nb19_sweep_summary_columns = [
    ...
    "fixed_nu_b", "all_u_xi_W_seed", "nu_b_eff_min", "nu_b_eff_max",
    "xi_W_eff_min", "xi_W_eff_max", "max_common_growth_log_gap",
    "exact_common_growth", "theta_strictly_feasible",
    "theta_one_percent_interior",
    "bound_status", "finite_envelope_diagnostics_pass",
