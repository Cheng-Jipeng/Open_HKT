# Included after 03_v9_ahp_pattern_T110_replication_zero_nu_b.jl. This file
# records the source-requested T=100 comparative statics and resolves them at
# the conservative zero-exponent horizon T=75.

const NB19_NFA_SOURCE_REQUESTED_T = 100
const NB19_NFA_SWEEP_T = 75
const NB19_NFA_SWEEP_HORIZONS = [30, 45, 60, 70, 75]
const NB19_NFA_SWEEP_BRANCH_ITER_SCHEDULE = [60, 200]
const NB19_NFA_SWEEP_MODE = Symbol(lowercase(get(
    ENV, "NB03_ZERO_NU_B_SWEEP_MODE", "full",
)))
NB19_NFA_SWEEP_MODE in (:full, :smoke) ||
    error("NB03_ZERO_NU_B_SWEEP_MODE must be full or smoke")
const NB19_NFA_OMEGA_STAR_GRID = NB19_NFA_SWEEP_MODE == :smoke ? [0.25] :
    [0.25, 0.30, 0.40, 0.50]
const NB19_NFA_OMEGA_GRID = NB19_NFA_SWEEP_MODE == :smoke ? [0.50] :
    [0.40, 0.50, 0.60, 0.70]
const NB19_NFA_OMEGA_STAR_CONTINUATION_GRID = NB19_NFA_SWEEP_MODE == :smoke ?
    Float64[] : Float64.(round.(collect(0.26:0.01:0.50); digits=2))
const NB19_NFA_OMEGA_UPPER_CONTINUATION_GRID = NB19_NFA_SWEEP_MODE == :smoke ?
    Float64[] : Float64.(round.(collect(0.51:0.01:0.70); digits=2))

println("\nZero-nu_b T=75 parameter sweeps for the U.S. NFA decomposition")
println("Source-requested horizon:       ", NB19_NFA_SOURCE_REQUESTED_T)
println("Resolved sweep horizon:         ", NB19_NFA_SWEEP_T)
println("Sweep mode:                      ", NB19_NFA_SWEEP_MODE)
...
function nb19_nfa_plot_feasible(result, validity)
    ...
    return validity.model_residuals && validity.psi_interior &&
           ...
           validity.phi_interior && validity.extended_bgp_valid &&
           validity.normalization_valid &&
...
function nb19_solve_nfa_sweep_case(omega_case::Float64,
    ...
    NB19_NFA_SWEEP_T in horizons || error("The NFA sweep must solve resolved T=75")
    ...
    for T in horizons
        ...
        for branch_iters in NB19_NFA_SWEEP_BRANCH_ITER_SCHEDULE
            ...
            try
                ...
                push!(attempts, Dict{String,Any}(
                    ...
                    "source_requested_T"=>NB19_NFA_SOURCE_REQUESTED_T,
                    "T_max"=>T,
        ...
        if solved_here === nothing && seed_result !== nothing
            polish_iters = 5
            started = time()
            try
                result = run_production_simulation(
        ...
        if solved_here === nothing
            ...
            return (;
                ...
                result=nothing, accounting=nothing, absorbing=nothing,
    ...
    absorbing = nb19_absorbing_normalization_summary(result)
    validity = nb19_hard_validity(result, accounting, absorbing)
    ...
    return (;
        ...
        result, accounting, absorbing, validity, plot_feasible,
        solved_by_T, attempts,
    )
end
...
function nb19_build_omega_nfa_cases()
    ...
    baseline.plot_feasible || error("The AHP resolved-T=75 baseline is unavailable")
    ...
    for value in NB19_NFA_OMEGA_STAR_CONTINUATION_GRID
        ...
        # If direct T=75 parameter continuation is not residual-safe, rebuild
        # the same parameter case through the validated zero-case horizon
        # ladder before retrying the resolved horizon.
...
nb19_nfa_attempt_columns = [
    ...
    "omega_bar", "omega_bar_star", "source_requested_T", "T_max", "branch_iters",
...
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "nfa_sweep_T75_solve_summary.csv"),
...
nb19_nfa_path_columns = [
    "sweep_kind", "sweep_value", "case_id", "label",
    "omega_bar", "omega_bar_star", "source_requested_T", "T_max", "t", "model_t",
...
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "nfa_sweep_T75_omega_star_paths.csv"),
    nb19_nfa_path_rows(omega_star_nfa_sweep_cases), nb19_nfa_path_columns,
)
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "nfa_sweep_T75_omega_paths.csv"),
...
nb19_nfa_case_summary_columns = [
    "sweep_kind", "sweep_value", "case_id", "label",
    "omega_bar", "omega_bar_star", "source_requested_T", "T_max", "solve_status",
    "failure_T", "failure_reason", "plot_feasible",
    "hard_valid_one_percent_gates", "equity_interior_one_percent",
    "theta_interior_one_percent", "phi_interior", "phi_lower_slack",
    "phi_upper_slack", "absorbing_normalization_valid",
    "absorbing_policy_invariance_error", "absorbing_aggregate_invariance_error",
    "absorbing_per_variety_scaling_error", "zero_exponent_compatibility_error",
    "automatic_growth_identity_error", "min_equity_weight",
...
omega_star_nfa_sweep_figure = nb19_plot_nfa_sweep(
    omega_star_nfa_sweep_cases;
    sweep_title="AHP-calibrated U.S. NFA decomposition: omega_bar* sweep, T=75",
    filename="nfa_sweep_T75_omega_star_12_panels.png",
)

omega_nfa_sweep_figure = nb19_plot_nfa_sweep(
    omega_nfa_sweep_cases;
    sweep_title="AHP-calibrated U.S. NFA decomposition: omega_bar sweep, T=75",
    filename="nfa_sweep_T75_omega_12_panels.png",
)

println("\nResolved T=75 U.S. NFA sweep validation")
