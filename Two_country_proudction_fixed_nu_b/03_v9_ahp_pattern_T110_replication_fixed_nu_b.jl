...
# Fixed-nu_b replication of the common-growth Notebook 19 workflow.  The
...
function nb19_find_project_dir(start_dir=pwd())
    ...
    while true
        for candidate in (dir, joinpath(dir, "Codes"))
            ...
            model = joinpath(candidate, "Two_country_proudction_fixed_nu_b",
...
const NB19_MODEL_FILE = joinpath(
    NB19_PROJECT_DIR, "Two_country_proudction_fixed_nu_b", "TwoCountryProductionOLG.jl",
)
if @isdefined ProductionParams
    @isdefined(_require_fixed_nu_b) || error(
        "A non-fixed TwoCountryProductionOLG solver is already loaded. " *
...
const NB19_OUTDIR = get(
    ENV, "NB03_FIXED_NU_B_OUTDIR",
    joinpath(
        NB19_PROJECT_DIR, "Two_country_proudction_fixed_nu_b", "outputs_fixed_nu_b",
        "03_ahp_pattern_T110_replication",
    ),
)
const NB19_TIME_INVARIANT_DIR = joinpath(
...
nb19_benchmark = nb19_read_numeric_csv(
    joinpath(NB19_TIME_INVARIANT_DIR, "best_ahp_decomposition.csv"),
)
nb19_benchmark_t = Int.(round.(nb19_benchmark["t"]))
nb19_benchmark_idx = findfirst(==(NB19_AHP_TARGET_END), nb19_benchmark_t)
nb19_benchmark_idx === nothing && error("Time-invariant benchmark has no t=15 row")
nb19_benchmark_Y = nb19_benchmark["Y_US"][nb19_benchmark_idx]

const NB19_AHP_TARGET = (
    rebased_nfa=nb19_benchmark["ahp_rebased_NFA_change_current_Y"][nb19_benchmark_idx],
    net_va=nb19_benchmark["ahp_cum_VA_current_Y"][nb19_benchmark_idx],
    ca=nb19_benchmark["ahp_cum_CA_current_Y"][nb19_benchmark_idx],
    asset_va=nb19_benchmark["cum_VA_row_equity_asset"][nb19_benchmark_idx] /
             nb19_benchmark_Y,
    liability_va=nb19_benchmark["cum_VA_us_equity_liability"][nb19_benchmark_idx] /
                 nb19_benchmark_Y,
)
...
function nb19_common_growth_diagnostics(result)
    ...
    for (j, b) in enumerate(result.bgp_seq_extended)
        ...
        G_W = b.G_N_W^p.ξ_W
        # Counterfactual exponent that would impose common growth at this
        # switch state.  It is never passed back to the fixed-nu_b solver.
        log_G_US = log(b.G_N_US)
        nu_b_common_growth_implied = abs(log_G_US) > 1e-12 ?
            p.ξ_W * log(b.G_N_W) / log_G_US : NaN
        push!(rows, Dict{String,Any}(
            "switch_index"=>j - 1,
            "G_N_US"=>b.G_N_US,
            "G_N_W"=>b.G_N_W,
            "nu_b_eff"=>b.ν_b_eff,
            "nu_b_common_growth_implied"=>nu_b_common_growth_implied,
...
function nb19_common_growth_summary(result)
    ...
    nu_cg = Float64[r["nu_b_common_growth_implied"] for r in rows]
    return (
        ...
        nu_b_common_growth_implied_min=minimum(nu_cg),
        nu_b_common_growth_implied_max=maximum(nu_cg),
...
function nb19_hard_validity(result, a, common)
    ...
    # `exact_common` is intentionally diagnostic only.  The fixed-nu_b model
    # is certified from its equilibrium, feasibility, accounting, and BGP
    # residuals without imposing G_b == G_W.
    valid = model_residuals && psi_interior && equity_interior && theta_interior &&
        ...
        finite_accounting && extended_bgp_valid && exponent_ordering
    return (
        valid=valid,
...
best_validity.valid || error("The fixed-nu_b T=110 continuation failed a model hard gate")
isapprox(best_result.params.ν_b, 1.0; atol=0.0, rtol=0.0) ||
    error("The fixed-nu_b solver did not preserve nu_b=1.0")
isapprox(best_result.params.ξ_W, 1.0; atol=0.0, rtol=0.0) ||
    error("The fixed-nu_b calibration did not preserve xi_W=1.0")
best_result.params.common_world_growth &&
    error("The common-growth restriction must remain disabled")
...
nb19_write_rows_csv(
    joinpath(NB19_OUTDIR, "best_fixed_nu_b_common_growth_reference.csv"),
    best_common_diagnostics,
    [
        "switch_index", "G_N_US", "G_N_W", "nu_b_eff",
        "nu_b_common_growth_implied", "bgp_converged",
...
p_ahp = plot(
    ...
    title="Fixed nu_b=1 AHP counterfactual at T=110: t=1:$(e) is the evidence window",
...
p_ratio = plot(
    xlabel="period t", ylabel="fraction of current U.S. output",
    title="Fixed nu_b=1: NFA/current Y (full T=110 tail)", legend=:outerbottom,
)
...
nu_cg = Float64[r["nu_b_common_growth_implied"] for r in best_common_diagnostics]
...
p_growth = plot(
    ...
    title="Growth factors without imposing common growth",
)
plot!(p_growth, switch_t, G_W, color=:red3, ls=:dash, lw=2.2, label="G_W")
p_nu = plot(
    switch_t, nu_eff; color=:purple, lw=2.6, label="fixed nu_b used by solver",
    xlabel="switch-state index", ylabel="effective exponent",
    title="Fixed exponent and common-growth reference",
)
plot!(p_nu, switch_t, nu_cg; color=:darkorange, ls=:dash, lw=2.2,
      label="common-growth-implied nu_b (reference only)")
hline!(p_nu, [best_result.params.ν_u], color=:gray35, ls=:dash, label="nu_u ceiling")
p_gap = plot(
    switch_t, growth_gaps; color=:black, lw=2.0, label="G_b - G_W",
    xlabel="switch-state index", ylabel="level gap",
    title="Common-growth gap (not a model restriction)",
)
fig_common_growth = plot(p_growth, p_nu, p_gap, layout=(1, 3), size=(1550, 460))
savefig(
    fig_common_growth,
    joinpath(NB19_OUTDIR, "best_fixed_nu_b_common_growth_reference.png"),
)

println("\n" * repeat("=", 88))
println("NOTEBOOK 03 — FIXED-NU_B AHP FIGURES ON THE T=110 CONTINUATION")
println(repeat("=", 88))
...
@printf("CG-implied reference nu_b range:  [%.6f, %.6f]\n",
        best_common_summary.nu_b_common_growth_implied_min,
        best_common_summary.nu_b_common_growth_implied_max)
